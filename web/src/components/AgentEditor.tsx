...
function resolveSectionEntries(
  ...
  for (const [key, property, prefix] of collected) {
    ...
    if (
      ...
      const groupFields = Object.entries(property.properties)
        .map(([childKey, childProperty]) => ({
          key: childKey,
...
const AgentEditor = React.forwardRef<AgentEditorHandle, AgentEditorProps>(
  function AgentEditor(
    ...
    const isDirty =
      canEdit && agent
        ? JSON.stringify(normalizeForDirtyCheck(draft)) !==
          JSON.stringify(
            normalizeForDirtyCheck(agent as unknown as Record<string, unknown>),
          )
        : false;
    useEffect(() => {
      ...
      return (
        ...
        >
          <Typography variant="body2" color="text.secondary">
            {t("agent.selectToEdit")}
          </Typography>
        </Box>
      );
    }
    const suggestionMap: Record<string, string[]> = {
      model: models.map((modelInfo) => `${modelInfo.providerName}:${modelInfo.id}`),
      ...suggestions,
    };
      ...
      <>
        ...
        {sections.map((section) => {
          const entries = schema
            ? resolveSectionEntries(schema, section, hidden)
            : [];
