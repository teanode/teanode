...
export default function ConversationsAllPage() {
  ...
  function isDefaultConversation(
    agentId: string,
  ...
  function handleSetDefault() {
    ...
    backend.setDefaultConversation(
      singleSelectedConversation.agentId,
      singleSelectedConversation.conversationId,
    );
    setSelected(new Set());
    setSelectMode(false);
  }
  ...
  return (
    <Box sx={{ flex: 1, overflowY: "auto" }}>
      <Container maxWidth="md">
        ...
        >
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 1 }}>
            <Typography
              variant="h6"
              sx={{ fontSize: "16px", fontWeight: 600, flex: 1, minWidth: 120 }}
            >
            ...
            {selectMode ? (
              <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
                ...
                {selected.size > 0 && (
                  <Tooltip
                    title={t("conversations.deleteSelected", {
                      count: selected.size,
                    })}
                  >
                    <Button
                    ...
                    >
                      <Box
                        ...
                        sx={{ display: { xs: "none", sm: "inline" } }}
                      >
                        {t("conversations.deleteSelected", {
                          count: selected.size,
                        })}
                      </Box>
                    </Button>
        ...
        <Box sx={{ pb: 2 }}>
          {Array.from(filteredByAgent.entries()).map(
            ([agentId, agentConversations]) => {
              const agent = agents.find(
                (agent: AgentInfo) => agent.id === agentId,
              );
                ...
                <Box key={agentId} sx={{ mb: 1.5 }}>
                  ...
                  >
                    ...
                    >
                      <IconButton
                        size="small"
                        onClick={() =>
                          navigate({ to: `/conversations/${agentId}` })
                        }
                        sx={{
                          p: 0,
                          color: "text.disabled",
                      ...
                      >
                        <AddIcon
                          sx={{ fontSize: 12, color: "text.disabled", mr: 0.5 }}
                        />
                        <Typography
                          variant="caption"
                          color="text.secondary"
                  ...
                  <List disablePadding>
                    {agentConversations.map((conversation) => {
                      const isDefault = isDefaultConversation(
                        agentId,
                        conversation.id,
                      );
                      ...
                      return (
                        <ListItemButton
                          ...
                          dense
                          onClick={() =>
                            handleConversationClick(agentId, conversation.id)
                          }
                        ...
                        >
                          ...
                          <ListItemText
                            primary={
                              trimmedFilter
                                ? highlightText(label, trimmedFilter)
                                : label
                            }
                            secondary={
                              <Box
                          ...
                          {isDefault && (
                            <Tooltip
                              title={t(
                                "conversations.defaultConversationLabel",
                              )}
                              placement="top"
                            >
      ...
      <ConfirmDialog
        ...
        message={t("conversations.deleteSelectedMessage", {
          count: selected.size,
        })}
