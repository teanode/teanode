...
function formatJobSchedule(job: Job, t: (key: string, options?: Record<string, unknown>) => string): string {
  if (job.oneShot) {
    if (job.runAt) return t('jobs.oneShotAt', { time: new Date(job.runAt).toLocaleString() });
...
export default function SettingsJobsPage() {
  ...
  const [oneShotMessageByJob, setOneShotMessageByJob] = useState<Record<string, string>>({});
  const [oneShotModelByJob, setOneShotModelByJob] = useState<Record<string, string>>({});
  const [oneShotAgentByJob, setOneShotAgentByJob] = useState<Record<string, string>>({});
  const [periodicDirtyByJob, setPeriodicDirtyByJob] = useState<Record<string, boolean>>({});
  ...
  const fallbackAgentId = backend.agents.length > 0 ? backend.agents[0].id : 'main';

  const viewConversation = useCallback((job: Job) => {
  ...
  const updateJobFromForm = useCallback((job: Job, data: { name: string; schedule: string; message: string; model: string; agentId: string }) => {
  ...
  const createJobFromForm = useCallback((data: { name: string; schedule: string; message: string; model: string; agentId: string }) => {
    ...
    backend.createJob(params).then(() => {
      setCreateFormKey((previous) => previous + 1);
      setCreateCanSave(false);
    }).catch(() => {});
  }, [backend.createJob]);

  const saveOneShotJob = useCallback((job: Job) => {
    ...
    backend.updateJob(params).catch(() => {});
  }, [backend.updateJob, oneShotMessageByJob, oneShotModelByJob, oneShotAgentByJob]);
    ...
    <Box sx={{ flex: 1, overflowY: 'auto' }}>
      <Container maxWidth="md" sx={{ py: { xs: 2, md: 3 } }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          <Box sx={{ mb: 1 }}>
            <Typography variant="subtitle1" sx={{ fontWeight: 600 }}>{t('settings.jobs')}</Typography>
            <Typography variant="caption" color="text.secondary">{t('settings.jobsDescription')}</Typography>
          </Box>
          {jobs.map((job) => {
            const currentMessage = oneShotMessageByJob[job.id] ?? job.message;
            const currentModel = oneShotModelByJob[job.id] ?? (job.model || '');
            const currentAgentId = oneShotAgentByJob[job.id] ?? (job.agentId || '');
            const oneShotDirty = job.oneShot && (
            ...
            <Paper key={job.id} variant="outlined" sx={{ p: 2 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1.5, mb: 1.5, py: 0.25 }}>
                <Box sx={{ minWidth: 0 }}>
                  <Typography
                    variant="subtitle2"
                    sx={{
                  ...
                  >
                    {job.oneShot ? t('jobs.onceLabel') : t('jobs.recurringLabel')}: {job.name}
                  </Typography>
                  <Typography variant="caption" color={job.enabled ? 'text.secondary' : 'text.disabled'}>
                    {formatJobSchedule(job, t)}
                  </Typography>
                </Box>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <Tooltip title={t('jobs.runNow')}>
                    <IconButton size="small" onClick={() => backend.triggerJob(job.id).then(() => viewConversation(job)).catch(() => {})}>
                      <PlayArrowIcon fontSize="small" />
                    </IconButton>
                  </Tooltip>
                  <Tooltip title={t('jobs.viewHistory')}>
                    <IconButton size="small" onClick={() => viewConversation(job)}>
                      <HistoryIcon fontSize="small" />
                    </IconButton>
                  </Tooltip>
                  <Tooltip title={job.enabled ? t('jobs.disableJob') : t('jobs.enableJob')}>
                    <IconButton size="small" onClick={() => backend.updateJob({ id: job.id, enabled: !job.enabled }).catch(() => {})}>
                      {job.enabled ? <ToggleOnIcon fontSize="small" /> : <ToggleOffIcon fontSize="small" />}
                    </IconButton>
                  </Tooltip>
                  ...
                  <Tooltip title={t('common.delete')}>
                    <IconButton size="small" color="error" onClick={() => setPendingDelete(job)}>
              ...
              {job.oneShot ? (
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75, pt: 0.5 }}>
                  <TextField
                    ...
                    onChange={(event) => {
                      ...
                      setOneShotMessageByJob((previous) => ({ ...previous, [job.id]: nextValue }));
                    }}
                  />
                  {backend.models.length > 0 ? (
                    <TextField
                      ...
                      fullWidth
                      value={currentModel}
                      onChange={(event) => setOneShotModelByJob((previous) => ({ ...previous, [job.id]: event.target.value }))}
                    >
                  ...
                  {backend.agents.length > 1 && (
                    <TextField
                      ...
                      onChange={(event) => setOneShotAgentByJob((previous) => ({ ...previous, [job.id]: event.target.value }))}
                    >
                ...
                <JobForm
                  ref={(instance) => { periodicFormRefs.current[job.id] = instance; }}
                  flat
                  showActions={false}
                  initial={job}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          backend.deleteJob(pendingDelete.id).then(() => setPendingDelete(null)).catch(() => {});
        }}
