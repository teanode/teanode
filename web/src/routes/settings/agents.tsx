...
import AgentEditor, { type AgentEditorHandle } from '../../components/AgentEditor';
...
async function uploadAgentAvatar(agentId: string, file: File): Promise<void> {
  ...
  const response = await fetch(`/api/v1/agents/${encodeURIComponent(agentId)}/avatar`, {
    method: 'POST',
    body: formData,
  });
...
export default function SettingsAgentsPage() {
  ...
  const [avatarBusyAgentId, setAvatarBusyAgentId] = useState<string | null>(null);
  ...
  const saveAgent = useCallback(
    (agent: AgentConfig) => agentsHook.saveAgent(agent).then(() => backend.refreshAgents()),
  ...
  const handleAvatarInput = useCallback(async (agentId: string, event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleAvatarDelete = useCallback(async (agentId: string) => {
    setAvatarBusyAgentId(agentId);
    try {
  ...
  const handleAgentDirtyChange = useCallback((agentId: string, dirty: boolean) => {
  ...
  const handleAgentNameChange = useCallback((agentId: string, name: string) => {
    ...
    editorRefs.current[agentId]?.setField('name', name.trim() ? name : undefined);
  }, []);

  const handleAvatarClick = useCallback((agent: AgentConfig) => {
    ...
    <Box sx={{ flex: 1, overflowY: 'auto' }}>
      <Container maxWidth="md" sx={{ py: { xs: 2, md: 3 } }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          <Box sx={{ mb: 1 }}>
            <Typography variant="subtitle1" sx={{ fontWeight: 600 }}>{t('settings.agents')}</Typography>
            <Typography variant="caption" color="text.secondary">{t('settings.agentsDescription')}</Typography>
          </Box>
          {sortedAgents.map((agent) => {
            const avatarBusy = avatarBusyAgentId === agent.id;
            const initial = (agent.name || agent.id).trim().charAt(0).toUpperCase();
            return (
              <Paper key={agent.id} variant="outlined" sx={{ p: 2 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: { xs: 1, sm: 2 } }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flex: 1, minWidth: 0 }}>
                    ...
                    >
                      <Box sx={{ position: 'relative' }}>
                        <Avatar
                          src={agent.avatarMediaId ? `/api/v1/media/${agent.avatarMediaId}` : undefined}
                          sx={{ width: 48, height: 48 }}
                        >
                          {avatarBusy ? <CircularProgress size={18} /> : initial}
                        </Avatar>
                    ...
                    <input
                      ...
                      type="file"
                      accept="image/*"
                      ref={(input) => { avatarInputRefs.current[agent.id] = input; }}
                    ...
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                      <TextField
                        ...
                        onChange={(event) => handleAgentNameChange(agent.id, event.target.value)}
                        InputProps={{ disableUnderline: true }}
                        sx={{
                          minWidth: { xs: 0, sm: 180 },
                          width: '100%',
                          maxWidth: '100%',
                          '& .MuiInputBase-input': { fontSize: '0.95rem', fontWeight: 600, py: 0.25 },
                        }}
                      />
                    </Box>
                  </Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: { xs: 0.5, sm: 1 }, flexShrink: 0 }}>
                    ...
                    <Tooltip title={t('agent.chat')}>
                      <IconButton
                        size="small"
                        onClick={() => navigate({ to: '/conversations/$agentId', params: { agentId: agent.id } })}
                      >
                    ...
                    <Tooltip title={t('common.delete')}>
                      <span>
                        <IconButton
                          size="small"
                          color="error"
                          onClick={() => setPendingDelete({ id: agent.id, name: agent.name || agent.id })}
                ...
                <Box sx={{ mt: 1.5 }}>
                  <AgentEditor
                    ref={(instance) => { editorRefs.current[agent.id] = instance; }}
                    agent={agent}
                    ...
                    onDirtyChange={(dirty) => handleAgentDirtyChange(agent.id, dirty)}
                    hiddenDotPaths={['name']}
                  />
                </Box>
              </Paper>
          ...
          <Paper variant="outlined" sx={{ p: 2 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 2 }}>
              ...
              <Button variant="contained" size="small" onClick={addAgent} disabled={!newAgentName.trim()}>
                {t('common.create')}
              </Button>
            </Box>
          </Paper>
