...
import type { AgentConfig, AgentsConfigListResult, AgentConfigSchemaResult, ConfigSchema } from '../types';
  ...
  const deleteAgent = useCallback(
    (id: string) => {
      return sendRpc('agents.config.delete', { id })
        .then(() => { loadAgents(); })
        .catch((error) => { console.error('agents.config.delete:', error); throw error; });
    },
    [sendRpc, loadAgents]
  );
