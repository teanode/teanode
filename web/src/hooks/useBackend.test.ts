...
class StreamingSimulation {
  ...
  startRun(runId: string, userText: string) {
    this.messages.push(
      { id: nextMessageId(), type: 'user', content: userText, timestamp: Date.now() },
    ...
    if (this.afterToolCalls) {
      ...
      if (previousText) {
        ...
        if (assistantIndex >= 0 && updated[assistantIndex].type === 'assistant') {
          updated[assistantIndex] = { ...updated[assistantIndex], content: previousText };
        }
        const newAssistant: DisplayMessage = { id: nextMessageId(), type: 'assistant', content: '', runId };
  ...
  toolCall(runId: string, toolName: string, args: string) {
    ...
    if (accumulatedText && assistantIndex >= 0 && updated[assistantIndex].type === 'assistant') {
      ...
      const newTail: DisplayMessage = { id: nextMessageId(), type: 'assistant', content: '', runId };
  ...
  final(runId: string, serverText?: string) {
    ...
    const hasToolSplits = updated.some((message, index) =>
      index !== assistantIndex && message.type === 'assistant' && message.runId === runId
    );
    ...
    if (assistantIndex >= 0 && updated[assistantIndex].type === 'assistant') {
      if (finalText) {
        updated[assistantIndex] = { ...updated[assistantIndex], content: finalText, timestamp: Date.now() };
      } else if (updated[assistantIndex].content) {
        // Assistant already has committed content — preserve it.
        updated[assistantIndex] = { ...updated[assistantIndex], timestamp: Date.now() };
      } else {
  ...
  renderedAssistantTexts(activeRunId: string): string[] {
    ...
    if (this.isStreaming) {
      for (let index = this.messages.length - 1; index >= 0; index--) {
        if (this.messages[index].type === 'assistant' && this.messages[index].runId === activeRunId) {
...
describe('streaming message merge', () => {
  ...
  it('preserves pre-tool text when a tool call arrives', () => {
    ...
    const assistants = simulation.messages.filter((message) => message.type === 'assistant');
  ...
  it('handles consecutive tool calls without text between them', () => {
    ...
    expect(nonEmpty.map((message) => message.content)).toEqual(['Intro', 'Done']);
  });
  ...
  it('preserves committed content when final has no post-tool text (two tool rounds)', () => {
    ...
    const assistants = simulation.messages.filter((message) => message.type === 'assistant');
    const nonEmpty = assistants.filter((message) => message.content);
    expect(nonEmpty.map((message) => message.content)).toEqual(['Before', 'Middle']);
  });
