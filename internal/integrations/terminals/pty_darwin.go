...
func MakeRaw(fileDescriptor int) (*TermState, error) {
	original, err := unix.IoctlGetTermios(fileDescriptor, unix.TIOCGETA)
	if err != nil {
		return nil, err
	}
	raw := *original
	...
	raw.Cc[unix.VMIN] = 1
	raw.Cc[unix.VTIME] = 0
	if err := unix.IoctlSetTermios(fileDescriptor, unix.TIOCSETA, &raw); err != nil {
		return nil, err
	}
...
func RestoreTermios(fileDescriptor int, state *TermState) {
	if state != nil && state.termios != nil {
		_ = unix.IoctlSetTermios(fileDescriptor, unix.TIOCSETA, state.termios)
	}
}
