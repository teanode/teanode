...
func (self *transaction) ModifySession(sessionId string, modifier func(*models.Session) error, options *store.Option) (*models.Session, error) {
	return self.modifySession(sessionId, modifier, options)
}
...
func (self *transaction) listSessions(options *store.Option) ([]models.Session, error) {
	...
	for _, entry := range entries {
		...
		results = append(results, sessionRecordToModel(record))
	}
	return applyOffsetLimitSessions(results, options), nil
}

func (self *transaction) createSession(session *models.Session, options *store.Option) (*models.Session, error) {
	if session == nil || session.UserID == nil || strings.TrimSpace(*session.UserID) == "" {
	...
	record := fileSystemSessionRecord{
		...
		RemoteAddr: valueOrEmpty(session.RemoteAddress),
		LastSeenAt: now,
	}
...
func modelToSessionRecord(session models.Session) fileSystemSessionRecord {
	record := fileSystemSessionRecord{
		...
		UserID:     strings.TrimSpace(valueOrEmpty(session.UserID)),
		UserAgent:  valueOrEmpty(session.UserAgent),
		RemoteAddr: valueOrEmpty(session.RemoteAddress),
	}
