...
func (self *transaction) ModifyJob(jobId string, modifier func(*models.Job) error, options *store.Option) (*models.Job, error) {
	return self.modifyJob(jobId, modifier, options)
}

func (self *transaction) DeleteJob(jobId string, options *store.Option) error {
	return self.deleteJob(jobId, options)
}
...
func (self *transaction) createJob(job *models.Job, options *store.Option) (*models.Job, error) {
	if job == nil || job.UserID == nil || strings.TrimSpace(*job.UserID) == "" {
	...
	now := time.Now()
	if createdJob.CreatedAt == nil {
		createdJob.CreatedAt = &now
	}
	createdJob.ModifiedAt = ptrto.Value(now)
...
func (self *transaction) modifyJob(jobId string, modifier func(*models.Job) error, options *store.Option) (*models.Job, error) {
	job, err := self.GetJob(jobId, options)
	if err != nil {
		return nil, err
	}
	...
	job.ID = jobId
	if job.UserID == nil || strings.TrimSpace(*job.UserID) == "" {
		return nil, store.ErrInvalidOptions
	}
	now := time.Now()
...
func formatJobMarkdown(job models.Job) []byte {
	...
	buffer.WriteString("---\n\n")
	buffer.WriteString(valueOrEmpty(job.Prompt))
	buffer.WriteString("\n")
	return buffer.Bytes()
}
...
func modelJobToFrontmatter(job models.Job) filesystemJobFrontmatter {
	frontmatter := filesystemJobFrontmatter{
		Name:           valueOrEmpty(job.Name),
		Schedule:       valueOrEmpty(job.Schedule),
		Model:          valueOrEmpty(job.Model),
		AgentID:        valueOrEmpty(job.AgentID),
		Enabled:        boolValue(job.Enabled),
		OneShot:        boolValue(job.OneShot),
		LastStatus:     valueOrEmpty(job.LastStatus),
		LastError:      valueOrEmpty(job.LastError),
		ConversationID: valueOrEmpty(job.ConversationID),
	}
	if job.RunAt != nil {
