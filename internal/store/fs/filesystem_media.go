...
func (self *transaction) ModifyMedia(mediaId string, modifier func(*models.Media) error, options *store.Option) (*models.Media, error) {
	return self.modifyMedia(mediaId, modifier, options)
}
...
func (self *transaction) listMedia(listOptions store.MediaListOptions, options *store.Option) ([]models.Media, error) {
	...
	for _, metadata := range metadataList {
		results = append(results, mediaMetadataToModel(metadata))
	}
...
func (self *transaction) createMedia(content io.Reader, metadata *models.Media, options *store.Option) (*models.Media, error) {
	...
	format := valueOrEmpty(metadata.Format)
	if format == "" {
		format = "bin"
	}
	...
	metadataRecord := storeMediaMetadata{
		...
		SourceType:     valueOrEmpty(metadata.Source),
		AgentID:        valueOrEmpty(metadata.SourceAgentID),
		ConversationID: valueOrEmpty(metadata.ConversationID),
		ToolName:       valueOrEmpty(metadata.ToolName),
		ToolCallID:     valueOrEmpty(metadata.ToolCallID),
		OriginalName:   valueOrEmpty(metadata.OriginalName),
	}
...
func (self *transaction) modifyMedia(mediaId string, modifier func(*models.Media) error, options *store.Option) (*models.Media, error) {
	_, metadata, err := self.GetMedia(mediaId, options)
	if err != nil {
		return nil, err
	}
