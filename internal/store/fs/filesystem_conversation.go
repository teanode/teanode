...
func (self *transaction) FindDefaultConversation(userId string, agentId string, options *store.Option) (*models.Conversation, error) {
	return self.findDefaultConversation(userId, agentId, options)
}

func (self *transaction) ModifyConversation(conversationId string, modifier func(*models.Conversation) error, options *store.Option) (*models.Conversation, error) {
	return self.modifyConversation(conversationId, modifier, options)
}

func (self *transaction) DeleteConversation(conversationId string, options *store.Option) error {
	return self.deleteConversation(conversationId, options)
}
...
func (self *transaction) listConversations(listOptions store.ConversationListOptions, options *store.Option) ([]models.Conversation, error) {
	...
	for _, entry := range entries {
		...
		conversationsList = append(conversationsList, conversation)
	}
...
func (self *transaction) findDefaultConversation(userId string, agentId string, options *store.Option) (*models.Conversation, error) {
	...
	conversationId := strings.TrimSpace(state.Users[userId].DefaultConversationIDs[agentId])
	if conversationId == "" {
		return nil, store.ErrNotFound
	}
	return self.GetConversation(conversationId, options)
...
func (self *transaction) modifyConversation(conversationId string, modifier func(*models.Conversation) error, options *store.Option) (*models.Conversation, error) {
	...
	if conversation.Title != nil || conversation.Summary != nil {
		updateError := self.updateConversationHeader(*conversation.UserID, *conversation.AgentID, conversationId, func(header *conversationFileHeader) {
			header.Title = valueOrEmpty(conversation.Title)
			header.Summary = valueOrEmpty(conversation.Summary)
