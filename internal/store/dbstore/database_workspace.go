...
func (self *databaseTransaction) CreateWorkspaceFile(ctx context.Context, file *models.WorkspaceFile, options *store.Option) (*models.WorkspaceFile, error) {
...
func (self *databaseTransaction) GetWorkspaceFileByPath(ctx context.Context, scope models.Scope, scopeId string, path string, options *store.Option) (*models.WorkspaceFile, error) {
	normalizedPath := normalizeWorkspacePath(path)
	record := &databaseWorkspaceFileRecord{}
	getError := self.database.Where("scope = ? AND scope_id = ? AND path = ?", string(scope), scopeId, normalizedPath).Take(record).Error
	if getError != nil {
		return nil, databaseError(getError)
	}
	return workspaceRecordToModel(record), nil
...
func (self *databaseTransaction) ModifyWorkspaceFileByPath(ctx context.Context, scope models.Scope, scopeId string, path string, modifier func(*models.WorkspaceFile) error, options *store.Option) (*models.WorkspaceFile, error) {
	file, getError := self.GetWorkspaceFileByPath(ctx, scope, scopeId, path, options)
	if getError != nil {
		return nil, getError
	}
	...
	file.ScopeID = ptrto.Value(scopeId)
	normalizedPath := normalizeWorkspacePath(path)
	file.Path = &normalizedPath
	return self.CreateWorkspaceFile(ctx, file, options)
}

func (self *databaseTransaction) DeleteWorkspaceFileByPath(ctx context.Context, scope models.Scope, scopeId string, path string, options *store.Option) error {
	normalizedPath := normalizeWorkspacePath(path)
	result := self.database.Where("scope = ? AND scope_id = ? AND path = ?", string(scope), scopeId, normalizedPath).Delete(&databaseWorkspaceFileRecord{})
	if result.Error != nil {
...
func (self *databaseTransaction) ListWorkspaceFilesByPath(ctx context.Context, scope models.Scope, scopeId string, path string, options *store.Option) ([]*models.WorkspaceFile, error) {
	normalizedPath := normalizeWorkspacePath(path)
	query := self.database.Model(&databaseWorkspaceFileRecord{}).Where("scope = ? AND scope_id = ?", string(scope), scopeId)
...
func (self *databaseTransaction) SearchWorkspaceFiles(ctx context.Context, scope models.Scope, scopeId string, query string, searchOptions store.WorkspaceSearchOptions, options *store.Option) ([]store.WorkspaceFileSearchResult, error) {
	listPath := ""
	if searchOptions.PathPrefix != nil {
		listPath = *searchOptions.PathPrefix
	}
	files, listError := self.ListWorkspaceFilesByPath(ctx, scope, scopeId, listPath, options)
	if listError != nil {
...
func (self *databaseTransaction) createSeedWorkspaceFiles(ctx context.Context, scope models.Scope, scopeId string, files []models.WorkspaceFile, options *store.Option) error {
	for _, file := range files {
		fileCopy := file
		fileCopy.Scope = ptrto.Value(scope)
		fileCopy.ScopeID = ptrto.Value(scopeId)
		if _, createError := self.CreateWorkspaceFile(ctx, &fileCopy, options); createError != nil {
			return createError
		}
	}
	return nil
