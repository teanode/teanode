...
func (self *databaseTransaction) ListMedia(ctx context.Context, listOptions store.MediaListOptions, options *store.Option) ([]*models.Media, error) {
	...
	if listOptions.Source != nil {
		query = query.Where("source = ?", *listOptions.Source)
	}
	if listOptions.ToolName != nil {
		query = query.Where("tool_name = ?", *listOptions.ToolName)
	}
...
func (self *databaseTransaction) CreateMedia(ctx context.Context, content io.Reader, metadata *models.Media, options *store.Option) (*models.Media, error) {
	if content == nil {
	...
	_, createdMedia, getError := self.GetMedia(ctx, record.ID, options)
	if getError != nil {
		return nil, getError
	}
...
func (self *databaseTransaction) GetMedia(ctx context.Context, mediaId string, options *store.Option) ([]byte, *models.Media, error) {
...
func (self *databaseTransaction) OpenMedia(ctx context.Context, mediaId string, options *store.Option) (io.ReadCloser, *models.Media, error) {
	record := &databaseMediaRecord{}
	getError := self.database.Where("id = ?", mediaId).Take(record).Error
	if getError != nil {
		return nil, nil, databaseError(getError)
...
func (self *databaseTransaction) ModifyMedia(ctx context.Context, mediaId string, modifier func(*models.Media) error, options *store.Option) (*models.Media, error) {
	existingRecord := &databaseMediaRecord{}
	getError := self.database.Where("id = ?", mediaId).Take(existingRecord).Error
	if getError != nil {
	...
	_, updatedMedia, updatedError := self.GetMedia(ctx, record.ID, options)
	return updatedMedia, updatedError
}

func (self *databaseTransaction) DeleteMedia(ctx context.Context, mediaId string, options *store.Option) error {
	record := &databaseMediaRecord{}
	getError := self.database.Where("id = ?", mediaId).Take(record).Error
	if getError != nil {
		return databaseError(getError)
	}
	result := self.database.Where("id = ?", mediaId).Delete(&databaseMediaRecord{})
	if result.Error != nil {
...
func modelToMediaRecord(media *models.Media) *databaseMediaRecord {
	return &databaseMediaRecord{
		ID:             media.ID,
		UserID:         ptrto.TrimmedString(media.GetUserID()),
		Format:         ptrto.TrimmedString(media.GetFormat()),
		ContentType:    ptrto.TrimmedString(media.GetContentType()),
		Source:         ptrto.TrimmedString(string(media.GetSource())),
		SourceAgentID:  ptrto.TrimmedString(media.GetSourceAgentID()),
		ConversationID: ptrto.TrimmedString(media.GetConversationID()),
		ToolName:       ptrto.TrimmedString(media.GetToolName()),
		ToolCallID:     ptrto.TrimmedString(media.GetToolCallID()),
		OriginalName:   ptrto.TrimmedString(media.GetOriginalName()),
		Size:           media.Size,
	}
}
...
func mediaRecordToModel(record *databaseMediaRecord) *models.Media {
	return &models.Media{
		...
		Format:         ptrto.TrimmedString(valueor.Zero(record.Format)),
		ContentType:    ptrto.TrimmedString(valueor.Zero(record.ContentType)),
		Source:         ptrto.Trimmed[models.MediaSource](valueor.Zero(record.Source)),
		SourceAgentID:  ptrto.TrimmedString(valueor.Zero(record.SourceAgentID)),
		...
		OriginalName:   ptrto.TrimmedString(valueor.Zero(record.OriginalName)),
		Size:           record.Size,
