...
func (self *databaseTransaction) ListConversations(listOptions store.ConversationListOptions, options *store.Option) ([]models.Conversation, error) {
	...
	if listOptions.AgentID != nil {
		query = query.Where("agent_id = ?", strings.TrimSpace(*listOptions.AgentID))
	}
...
func (self *databaseTransaction) CreateConversation(conversation *models.Conversation, options *store.Option) (*models.Conversation, error) {
	if conversation == nil {
	...
	record.CreatedAt = valueOrTime(conversation.CreatedAt)
	record.ModifiedAt = valueOrTime(conversation.ModifiedAt)
	createError := self.database.Create(record).Error
	if createError != nil {
		return nil, databaseError(createError)
	}
	return self.GetConversation(record.ID, options)
}

func (self *databaseTransaction) GetConversation(conversationId string, options *store.Option) (*models.Conversation, error) {
	record := &databaseConversationRecord{}
	getError := self.database.Where("id = ?", strings.TrimSpace(conversationId)).Take(record).Error
	if getError != nil {
...
func (self *databaseTransaction) FindDefaultConversation(userId string, agentId string, options *store.Option) (*models.Conversation, error) {
	record := &databaseConversationRecord{}
	getError := self.database.Where("user_id = ? AND agent_id = ? AND \"default\" = ?", strings.TrimSpace(userId), strings.TrimSpace(agentId), true).Order("modified_at DESC").Take(record).Error
	if getError != nil {
		return nil, databaseError(getError)
	}
	return conversationRecordToModel(record), nil
...
func (self *databaseTransaction) ModifyConversation(conversationId string, modifier func(*models.Conversation) error, options *store.Option) (*models.Conversation, error) {
	conversation, getError := self.GetConversation(conversationId, options)
	if getError != nil {
		return nil, getError
	}
	...
	record := modelToConversationRecord(conversation)
	record.ID = strings.TrimSpace(conversationId)
	record.ModifiedAt = time.Now().UTC()
	if conversation.CreatedAt != nil {
		record.CreatedAt = conversation.CreatedAt.UTC()
	}
...
func (self *databaseTransaction) DeleteConversation(conversationId string, options *store.Option) error {
	result := self.database.Where("id = ?", strings.TrimSpace(conversationId)).Delete(&databaseConversationRecord{})
	if result.Error != nil {
...
func modelToConversationRecord(conversation *models.Conversation) *databaseConversationRecord {
	return &databaseConversationRecord{
		...
		UserID:  ptrto.TrimmedString(valueOrEmptyString(conversation.UserID)),
		AgentID: ptrto.TrimmedString(valueOrEmptyString(conversation.AgentID)),
		Default: conversation.Default,
		Title:   ptrto.TrimmedString(valueOrEmptyString(conversation.Title)),
		Summary: ptrto.TrimmedString(valueOrEmptyString(conversation.Summary)),
	}
}
...
func conversationRecordToModel(record *databaseConversationRecord) *models.Conversation {
	return &models.Conversation{
		...
		Default:    record.Default,
		Title:      ptrto.TrimmedString(valueOrEmptyString(record.Title)),
		Summary:    ptrto.TrimmedString(valueOrEmptyString(record.Summary)),
