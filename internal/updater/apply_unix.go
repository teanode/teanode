...
func platformApply(currentPath, stagedPath string) error {
	...
	if err := renameOrCopy(stagedPath, currentPath); err != nil {
		...
		if restoreError := os.Rename(backupPath, currentPath); restoreError != nil {
			return fmt.Errorf("updater: CRITICAL: failed to restore backup after apply error: apply=%w, restore=%v", err, restoreError)
		}
		return fmt.Errorf("updater: applying update: %w (backup restored)", err)
	}
...
func copyFile(source, destination string) error {
	...
	if err != nil {
		return fmt.Errorf("updater: opening staged binary: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("updater: creating temp file for copy: %w", err)
	}
	...
	if _, err := io.Copy(temporaryFile, sourceFile); err != nil {
		...
		return fmt.Errorf("updater: copying staged binary: %w", err)
	}
	...
	if err := os.Rename(temporaryPath, destination); err != nil {
		_ = os.Remove(temporaryPath)
		return fmt.Errorf("updater: moving temp file into place: %w", err)
	}
